\chapter{Algebraic Integers}
	...
	\section{Galois extensions}
		...
		\begin{example}
				Let's study the prime ideals in the Gaussian integers $\Z[i]$ lying above the ideal $(5)$ in the ordinary integers $\Z$. As $(1+2i)(1-2i)=5$, the ideals $(1+2i)$ and $(1-2i)$ in $\Z[i]$ lies above the ideal $(5)$ in $\Z$. It's possible to show that they are the only such prime ideals. As the ring of Gaussian integers is a principal ideal domain (PID) \href{https://proofwiki.org/wiki/Ring_of_Gaussian_Integers_is_Principal_Ideal_Domain}{[proof]}, any such ideal is generated by a complex number of the form $a+bi$ with $a$ and $b$ integers. The squared complex norm of $a+bi$, i.e. $a^2+b^2$ must be divisible by $25$, as the squared complex norm of any element in $(a+bi)$ is an integer multiple of that of $a+bi$. It turns out that we only need to investigate a finite set of $a+bi$s, as neither of $|a|$ and $|b|$ exceeds $5$.
				
				The result is that the only possibilities of the ideal $(a+bi)$ are $(5)$, $(1+2i)$ and $(1-2i)$, apart from the trivial ideal $(1)$. $(5)$ is not prime because $(1+2i)(1-2i)=5$. The other two ideals are prime, becuase the rings $Z[i]/(1+2i)$ and $Z[i]/(1-2i)$ both have $5$ elements and are generated by $1$ (this is also a finite calculation, as we only need to consider the representatives $a+bi \in \Z[i]$ with $a,b \in \{0,1,2,3,4\}$), so the quotient rings are both isomorphic to the finite field on $5$ elements. As the quotient rings are integral, the ideals are prime.
				
				Now let's study the action of the Galois group on the ideals. The Galois group of $\Q[i]$ over the rationals consists of two elements (as it's a extension of degree 2), and it's generated by complex conjugation $\sigma$. The map $\sigma$ sends $(1+2i)$ to $(1-2i)$ and vice versa, thus the Galois group acts transitively on the ideals above $(5)$.
				
		\end{example}
		
		The idea of squared complex norm in this example can be generalized to the notion of norm in general Dedekind domains: See the subsection **Norm** in **Dedekind domain**.
