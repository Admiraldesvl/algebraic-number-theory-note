	...
	\section{Galois extensions}
		...
		\begin{example}
			...
			\[
				5+0i = (2-i)(2+i).
			\]
			Since $\Z[i]$ is a PID, we only need to consider Gaussian integers that divide $5$. Keeping in mind that $\Z[i]$ is also an Euclidean domain, we have two non-trivial solutions up to $\pm 1$:
			\[
				5 = (2-i)(2+i)=(1+2i)(1-2i).
			\]
			
			As ideals, $(2-i) = (1+2i)$ because we can define an isomorphism $\varphi:(2-i) \to (1+2i)$ by $z \mapsto iz$ while $\varphi$ is also an automorphism. Since $2-i$ is irreducible in $\Z[i]$, we have found a prime ideal $\mfk{P}=(2-i)$ lies above $\mfk{p}$, and $\mfk{P}'=\sigma\mfk{P}=(2+i)$ has to be the remaining one. Note we have also established the factorisation of $5\Z[i]$ in the Dedekind domain $\OK$.
